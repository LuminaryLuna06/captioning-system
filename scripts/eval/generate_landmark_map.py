...
def suggest_mapping(names: list[str], kb_nodes: dict) -> dict:
    ...
    for name in names:
        matches = get_close_matches(name.lower(), kb_by_lower.keys(), n=1, cutoff=0.55)
        if matches:
            result[name] = {"kb_id": kb_by_lower[matches[0]], "in_kb": True}
        else:
            result[name] = {"kb_id": None, "in_kb": False}
    return result
...
def main():
    ...
    mapping = suggest_mapping(names, kb_nodes)
    ...
    if null_entries:
        print(f"WARNING: {len(null_entries)} in-KB entries have no kb_id match — review manually:")
